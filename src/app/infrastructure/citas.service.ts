...
export class CitasService {
  private apiUrl = environment.apiBaseUrl + '/api/citas'; // Ajusta si necesitas proxy
  ...
  clearCache() {
    ...
    // 3) Limpiar TODAS las llaves per-combinaciÃ³n que empiecen con el prefijo
  ...
  // ======================================================
  //   API pÃºblica original (stats, etc.)
  // ======================================================
  ...
  /** ðŸ”Ž Buscar citas por filtros (nuevo) */
      ...
      .pipe(
        map(resp => {
          resp?.data?.forEach((cita: Cita) => {
            if (cita.tipo_de_entrega?.trim().toLowerCase() === 'operador logÃ­sitico' ||
              cita.tipo_de_entrega?.trim().toLowerCase() === 'operador logistico' ||
              cita.tipo_de_entrega?.trim().toLowerCase() === 'operador logÃ­stico') {
              cita.tipo_de_entrega = 'Operador LogÃ­stico';
            }
            /*if (cita.unidad?.trim().length == 0) {
              cita.unidad = this.mapCluesUnidad.get(cita.clues_destino) ?? '';
            }*/
            if (cita.unidad?.trim() == 'AlmacÃ©n Zona Ensenada' ||
              cita.unidad?.trim() == 'ALMACEN ZONA ENSENADA') {
              cita.unidad = 'ALMACÃ‰N ZONA ENSENADA';
            }
            if (cita.unidad?.trim() == 'ALMACÃ‰N DE MEXICALI' ||
              cita.unidad?.trim() == 'ALMACEN DE MEXICALI') {
              cita.unidad = 'ALMACÃ‰N ZONA MEXICALI';
            }
            if (cita.unidad?.trim() == 'ALMACEN TIJUANA' ||
              cita.unidad?.trim() == 'ALMACÃ‰N TIJUANA' ||
              cita.unidad?.trim() == 'ALMACEN ZONA TIJUANA') {
              cita.unidad = 'ALMACÃ‰N ZONA TIJUANA';
            }
            if (cita.unidad?.trim() == 'UNEME DE ONCOLOGIA' ||
              cita.unidad?.trim() == 'UNEME ONCOLOGIA' ||
              cita.unidad?.trim() == 'UNEME ONCOLOGÃA'
            ) {
              cita.unidad = 'UNEME DE ONCOLOGÃA';
            }
  ...
  /**
   * Construye una llave de cache en base a la combinaciÃ³n maestra de filtros.
  ...
  private buildCacheKey(params: SearchCitasParams): string {
    ...
    const normArr = (arr?: (string | number)[]) =>
      (arr ?? [])
        .map(x => norm(x))
        .filter(x => !!x)
        .sort()                 // ðŸ‘ˆ importante para que [A,B] == [B,A]
        .join(',');
  ...
  /**
   * Buscar citas por filtros (con cachÃ©).
   *
   * Esta funciÃ³n es similar a `searchCitas` pero utiliza un cachÃ© en memoria (Map<string, CitasCacheEntry>)
   * para evitar requests innecesarios al backend.
   *
   * Si se encuentra una entrada en el cachÃ© con un timestamp reciente (menos de `CACHE_TTL_MS` milisegundos)
   * se devuelve directo el resultado desde el cachÃ© sin ir al backend.
   *
   * Si no se encuentra una entrada en el cachÃ© o estÃ¡ vencido, se va al backend y se actualiza el cachÃ©.
   *
   * La cachÃ© se guarda en memoria y se persiste en `localStorage`.
   *
   * @param params Filtros para buscar las citas
   * @param opts Opciones extras; si { forceRefresh: true } se fuerza la recarga del cachÃ©
  ...
  searchCitasCached(
        ...
        if (raw) {
          ...
          if (decompressed) {
            ...
            if (this.isCacheValid(parsed) && parsed.data) {
              console.log('ðŸ‘‰ Devolviendo citas desde cache');
              // ðŸ‘‡ Devolvemos observable â€œfrÃ­oâ€ desde cache
    ...
    // ðŸ‘‰ Si no hay cache vÃ¡lido (o forceRefresh = true), llamamos al backend
    let hp = new HttpParams();
    const appendMany = (k: string, arr?: (string | number)[]) =>
      (arr ?? []).forEach(v => hp = hp.append(k, String(v)));
    ...
    return this.http.get<CitaQueryResponse>(`${this.apiUrl}`, { params: hp })
      .pipe(
        map(resp => {
          // ðŸ‘‡ tu normalizaciÃ³n actual (tipo_de_entrega, unidad, fechas, etc.)
          resp?.data?.forEach((cita: Cita) => {
            if (cita.tipo_de_entrega?.trim().toLowerCase() === 'operador logÃ­sitico' ||
              cita.tipo_de_entrega?.trim().toLowerCase() === 'operador logistico' ||
              cita.tipo_de_entrega?.trim().toLowerCase() === 'operador logÃ­stico') {
          ...
          console.log('ðŸ‘‰ Devolviendo citas desde backend y actualizando cache');
      ...
      try {
        ...
        for (const popo of rows) {
          ...
          if (!ejercicio || (ejercicio + '').trim().length === 0) {
            console.info('ðŸ” fin de archivo detectado en renglÃ³n ' + renglon + '. Finalizando obtenciÃ³n de datos', fila);
            break;
          }
          ...
          // Condiciono a que la fecha de recepciÃ³n siempre sea null
          // si no tiene numero de remision (fila[22]) porque estÃ¡n intimamente ligados
        ...
        // console.info(`âœ… Datos cargados desde Power Automate. Total: ${citasRetorno.length} registros.`);

        corriendoCicloCitas = true;
        // creando rapidamente un map para relacion entre clues_destino y unidad
        // donde unidad no tenga valor vacÃ­o
        ...
        // En tipo_de_entrega reemplacemos la palabra "operador logÃ­sitico" por "operador lÃ³gistico"
        citasRetorno.forEach((cita: Cita) => {
          citaProcesando = cita;
          if (cita.tipo_de_entrega?.trim().toLowerCase() === 'operador logÃ­sitico' ||
            cita.tipo_de_entrega?.trim().toLowerCase() === 'operador logistico' ||
        ...
        console.error('âŒ CitasService.obtenerCitasDePowerAutomate() - Error al obtener de power automate:', err);
        if (corriendoCiclo) {
          console.error('ðŸ” CitasService.obtenerCitasDePowerAutomate() - Error Procesando fila:', fila);
        } else if (corriendoCicloCitas) {
          console.error('ðŸ” CitasService.obtenerCitasDePowerAutomate() - Error Procesando cita:', citaProcesando);
        }
      }
      return citasRetorno;
    } */
